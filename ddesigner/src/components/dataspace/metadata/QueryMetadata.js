...
class QueryMetadata extends Component {
    ...
    renderStringField(value, disabled = false, nestedValue = null) {
        ...
        return <Debounced
            id={value} value={val}
            disabled={disabled}
            Component={TextArea} autosize={{minRows: 2, maxRows: 40}}
            onChange={e =>
                nestedValue ?
    ...
    render() {
        return [
            ...
            <FormItem label={this.getFieldLabel('query')} key={this.getFieldLabel('query')}>
                {this.renderStringField('query')}
            </FormItem>]
    }
}

export default translate()(QueryMetadata);