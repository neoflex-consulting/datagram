...
class TransformationTest extends GenerationBase {
    ...
    static Object runPart(Map entity, Map params) {
        ...
        String deploymentDir = gitFlow.SOURCES + "/Transformation/" + transformation.name
        return gitFlow.inCopy(deploymentDir, null, new Function<File, Object>() {
            @Override
            Object apply(File dir) {
                File file = new File(dir, "src/test/scala/" + transformation.name + trTest.name + "Test.scala")
                file.delete()
                file.write(entity.get("fileContent"))
                Context.current.getContextSvc().getMavenSvc().run(new File(dir, "pom.xml"), "clean,test", null,
                        null, null, ["suites":"ru.neoflex.meta.etl2.spark." + transformation.name + trTest.name + "Test"])
            }
        })
    }
    ...
    static Object generate(Map entity, Map params){
        ...
        return gitFlow.inDir(sourcesDir, "Generate Test", new BiFunction<Path, Path, Map>() {
            @Override
            Map apply(Path tmp, Path sourcesPath) {
                File file = new File(tmp.toFile(), transformation.name + "/src/test/scala/" + transformation.name + trTest.name + "Test.scala")
                if(file.exists()){
                    file.delete()
                }
                ...
                Context.current.getContextSvc().epsilonSvc.executeEgx("/psm/etl/spark/TransformationTest.egx", [mspaceRoot: tmp.toUri().toString(),  nodeName: params.nodeName, outputType: params.outputType], [emfModel])
                gitFlow.copyContentRecursive(tmp.resolve(transformation.name), sourcesPath.resolve(transformation.name), false)
    ...
    static Object run(Map entity, Map params){
        ...
        def gitFlow = Context.current.getContextSvc().getGitflowSvc()
        def sourcesDir = gitFlow.SOURCES + "/Transformation"
        return gitFlow.inCopy(gitFlow.SOURCES + "/Transformation/" + transformation.get("name") as String, null, new Function<File, Object>() {
            @Override
            Object apply(File tmp) {
                def buildFile = new File(tmp, "pom.xml")
                Map map = new HashMap()
                def packagePrefix = "ru.neoflex.meta.etl2.spark"
                if(trTest.enabled){
                    map.put("suites", packagePrefix + "." + transformation.name + trTest.name + "Test")
                    Context.current.getContextSvc().getMavenSvc().run(buildFile, "clean,test", null, null, null, map)
    ...
    static Object runPartOld(Map entity, Map params = null) {
    /* protected region MetaServer.etlTransformation.runit on begin */
        def tr = Database.new.get(entity)
        def trd = findOrCreateTRD(tr)
        Context.current.commit()
