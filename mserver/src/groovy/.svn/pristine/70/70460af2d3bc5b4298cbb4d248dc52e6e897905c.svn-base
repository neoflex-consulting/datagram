...
class TransformationTest extends GenerationBase {
    ...
    public static List collectTrDeployments(Map tr, List collected) {
    ...
    public static Map findOrCreateAutogeneratedProject() {
    ...
    public static Map findOrCreateWF(name, Map entity) {
        def db = Database.new
    ...
    public static Object runPart(Map entity, Map params){

        logger.info("public static Object runPart(Map entity, Map params)");
        def deploymentDir = getDirectoryPath("sources", "Transformation")
        logger.info("CONTEXT " + deploymentDir)
        def trTest = new Database("teneo").get("etl.TransformationTest", (Long) entity.e_id)
        def transformation = trTest.transformation
        def deployment = findOrCreateTRD(transformation);
        def emfModel = new EmfModel()
        def properties = new StringProperties()
        def fileName = deploymentDir + "/" + transformation.name + "/src/test/scala/" + trTest.name + "Test.scala"
        File file = new File(fileName)
        file.delete()
        file = new File(fileName);
        file.write(entity.get("fileContent"));
        Map map = new HashMap()
        def packagePrefix = "ru.neoflex.meta.etl2.spark"
        map.put("suites", packagePrefix + "." + transformation.name + trTest.name + "Test");
        fileName = deploymentDir + "/" + transformation.name + "/pom.xml"
        Context.current.getContextSvc().getMavenSvc().run(new File(fileName), "clean,test", null, null, null, map);


    }

    public static Object generate(Map entity, Map params){
        logger.info("public static Object run(Map entity, Map params)");
        def sourcesDir = getSourcesDirectoryPath("Transformation")
        logger.info("CONTEXT " + sourcesDir)
        def trTest = new Database("teneo").get("etl.TransformationTest", (Long) entity.e_id)
        def transformation = trTest.transformation
        def emfModel = new EmfModel()
        def properties = new StringProperties()
        def fileName = sourcesDir + "/" + transformation.name + "/src/test/scala/" + transformation.name + trTest.name + "Test.scala"
        File file = new File(fileName)
        if(file.exists()){
            file.delete()
        }
        properties.put(EmfModel.PROPERTY_NAME, "src")
        properties.put(EmfModel.PROPERTY_MODEL_URI, "hibernate://?dsname=teneo&query1=from etl.TransformationTest where e_id=${trTest.e_id}")
    ...
    public static Object run(Map entity, Map params){
        ...
        if(trTest.enabled){
            ...
            return [result: true, message : "Test is disabled"]
        }
        Context.current.getContextSvc().getMavenSvc().run(new File(buildFile), "clean,test", null, null, null, map);

    }



    public static Object checkTRD(Map transformation, Map params = null) {
        def db = Database.new
    ...
    public static Map findOrCreateTRD(Map transformation) {
        return findOrCreateTRD("autogenerated_tr_" + transformation.name, transformation)
    }

    public static Map findOrCreateTRD(name, Map transformation) {
        def db = Database.new
        def trdList
    ...
    public static Map createTRD(name, Map transformation) {
    ...
    public static Object validateModel(Map entity) {
        ...
        def properties = new StringProperties()
        properties.put(EmfModel.PROPERTY_NAME, "src")
        properties.put(EmfModel.PROPERTY_MODEL_URI, "hibernate://?dsname=teneo&query1=from etl.TransformationTest where e_id=${entity.e_id}")
        properties.put(EmfModel.PROPERTY_METAMODEL_URI, "http://www.neoflex.ru/meta/etl")
        properties.put(EmfModel.PROPERTY_READONLOAD, "true")
        emfModel.load(properties, "" );
        def problems = []
    ...
    public static Object expandXMLFile(Map entity, Map params = null) {
      def result = []
    ...
    public static Object expandAvroFile(Map entity, Map params = null) {
      def result = []
      def tr = Database.new.get(entity)
      def trd = findOrCreateTRD(tr)
      Context.current.commit()
    ...
    public static Object importTransformation(Map entity, Map params = null) {
        def transformation = Database.new.get(entity)
        def project = transformation.project
        if (project == null) {
            throw new RuntimeException("Project not found")
        }
        return Project.importProjectEntity(project, transformation)
    }

    public static Object exportTransformation(Map entity, Map params = null) {
        def transformation = Database.new.get(entity)
        def project = transformation.project
        if (project == null) {
            throw new RuntimeException("Project not found")
    ...
    /* protected region MetaServer.etlTransformation.runit on begin */
        ...
        Context.current.commit()
        return TransformationDeployment.generateAndRun(trd, entity.params)
    /* protected region MetaServer.etlTransformation.runit end */
    }

    public static Object runPartOld(Map entity, Map params = null) {
    /* protected region MetaServer.etlTransformation.runit on begin */
        def tr = Database.new.get(entity)
        def trd = findOrCreateTRD(tr)
        Context.current.commit()
    ...
    public static Object partJobFile(Map entity, Map params = null) {
    /* protected region MetaServer.etlTransformation.runit on begin */
      def result = []
      def tr = Database.new.get(entity)
      def trd = findOrCreateTRD(tr)
    ...
    public static Object mainJobFile(Map entity, Map params = null) {
    /* protected region MetaServer.etlTransformation.runit on begin */
    	def result = []
      def tr = Database.new.get(entity)
      def trd = findOrCreateTRD(tr)
    ...
    public static Object writeMainJobFile(Map entity, Map params = null) {
    /* protected region MetaServer.etlTransformation.runit on begin */
    	def result = []
    ...
    public static Object nodeDependencies(Map entity, Map params = null) {
