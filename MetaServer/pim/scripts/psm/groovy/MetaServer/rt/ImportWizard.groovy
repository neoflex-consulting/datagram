...
class ImportWizard {
        ...
        try {
            ...
            try {
                while (tabSet.next()) {
                    ...
                    def activeEntity = importWizard.entities.find {it.active = true}
                    def active_ = true;
                    if(activeEntity != null && activeEntity.size() > 0 ){
                        active_ = false;
                    }
    ...
    public static Object uploadSpecification(Map entity, Map params){

        def importWizard = Database.new.get(entity)

        def file = params.file
        if (file == null) {
            return ["Import Wizard CSV file not found"]
        }

        logger.info("Import Wizard upload specification : " + file.filename);


        byte[] buffer = new byte[4096];
        def fileInputStream = file.inputStream as InputStream;
        def newFile = new File(BaseSvc.getTempDir(), file.filename);
        if(newFile.exists()){
            newFile.delete();
            logger.info("Import Wizard Delete existing file : " + newFile.getAbsolutePath());
            newFile = new File(BaseSvc.getTempDir(), file.filename);
        }
        OutputStream outputStream = new BufferedOutputStream(new FileOutputStream(newFile), buffer.length);
        try {
            int length;
        ...
        newFile.withReader { reader ->
            while ((line = reader.readLine()) != null) {
                def cols = line.split(";");
                def tableName = cols[0];
                def schemaName = cols[1]
                def catName = cols[2]
                def type = cols[3]
                def importEntity = importWizard.entities.find {name == tableName && schema == schemaName}
                if (importEntity == null) {
                    def importEntityType = JSONHelper.getEnumerator("teneo", "rt.ImportEntity", "importEntityType", type)
                    importEntity = Database.new.instantiate("rt.ImportEntity", [
                            name: tableName,
                            schema: schemaName,
                            importEntityType: importEntityType,
                ...
                tables.add([TABLE_CAT: catName, TABLE_SCHEM: schemaName, TABLE_NAME: tableName, TABLE_TYPE: type])
                lines++
            }
        }
        logger.info("Import Wizard UploadSpecification csv lines processed : "  + lines);
