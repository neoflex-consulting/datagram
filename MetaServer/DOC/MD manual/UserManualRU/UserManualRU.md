...
2. Отредактируйте файл ldap.properties (см. "[Учетные записи пользователей](#user_accounts)"):
   ...
   | ------------- | --------------------- | :----------------------------------------------------------- |
   | ldap.domain   | Да                    | Доменное имя Ldap сервера.<br><br>*Для авторизации по LDAP используется userPrincipalName вида: username@domainname.com. Если поле не заполнено, то при авторизации необходимо указывать userPrincipalName полностью. Если domain указан, то допускается ввод только userName* |
   | ldap.host     | Да                    | Имя хоста Ldap сервера                                       |
   | ldap.port     | Да                    | Порт Ldap сервера                                            |
   | ldap.base     | Да                    | Путь к каталогу для поиска пользователей                     |
   | ldap.admin    | Да                    | Имя группы пользователей, которым будут предоставлены права администратора<sup>1</sup> |
   | ldap.operator | Да                    | Имя группы пользователей, которым будут предоставлены права оператора<sup>2</sup> |
   | ldap.user     | Да                    | Имя группы пользователей, которым будут предоставлены права пользователя |

   

3. Запустите файл mserver-*version number*-SNAPSHOT.jar при помощи стандартной команды запуска JAR-файлов Linux: **java -Dparameter=value ... -jar ${JAR_NAME}**, с указанием параметров:
...
| -Dmspace.dir         | Да                    | Путь к каталогу, в котором хранятся ресурсы слоя внедрения (см. раздел "[Ресурсы программы](#program_resources)") |
| -Ddeploy.dir         | Нет                   | Путь к каталогу, в котором хранятся ресурсы слоя сопровождения. Если параметр не задан, то каталог формируется по умолчанию: ${mspace.dir}/deployments/{cust.code} (см. раздел "[Ресурсы программы](#program_resources)") |
...
| -Dldap.config        | Нет                   | Путь к файлу конфигурации ldap (см. раздел "[Учетные записи пользователей](user_accounts)") |
| -Dpasswords          | Нет                   | Путь к файлу хранения паролей. По умолчанию: ${user.dir}/passwords.properties (см. раздел «[Хранение и шифрование паролей в системе](#storing_and_encrypting_passwords)») |
| -Ddencrypt.passwords | Нет                   | Опция шифрования паролей. Может принимать два значения: «false» (установлено по умолчанию) и true   (см. раздел «[Хранение и шифрование паролей в системе](#storing_and_encrypting_passwords)») |



4. Запустите браузер и в адресной строке введите строку:
...
| is Default           | Нет                       | При включенном параметре, объект "Oozie" будет использоваться по умолчанию при запуске на исполнение объектов "Workflow".<br><br>*В случае если в Neoflex Datagram создано несколько объектов "Oozie", и у всех включен параметр is default, то для исполнения объекта "Workflow", объект "Oozie" будет выбран случайно* |
...
| Http                 | Да                        | Url-адрес Livy Server API<br><br>*Пример: http://cloud.neo.ru:8090* |
...
В качестве источника данных используется файл, содержащий данные в формате CSV.
...
| Header               | Нет                  | Если параметр включен, то при извлечении данных из файла будет пропускаться первая строка (используется, если в CSV используется заголовок) |
...
| Date format          | Нет                  | Формат даты, используемый в файле-источнике<br><br>*Пример: dd.mm.yyyy* |
...
| Treat Empty Values As Nulls| Нет            | Если параметр включен, то при чтении пустые значения будут определены как 0 |
...
| Events processor           |            Нет            | Объекта Events processor, используемый для обработки входящего потока данных |
...
В качестве приемника данных используется текстовый файл, в который записывается результат трансформаций в формате CSV.
...
| Charset              | Да	                  | Кодировка, используемая в файле |
| Header               | Нет                  | Если параметр включен, то при записи данных в файл будет пропускаться первая строка |
| Delimiter            | Да                   | Символ, используемый в качестве разделителя между значениями в CSV |
| Quote                | Нет                  | Символ, предназначенный для выделения значения, содержащего символы Delimiter |
| Escape               | Нет                  | Символ, предназначенный для выделения значения, содержащего символы Quote |
| Comment              | Нет                  | Символы, предназначенные для обозначения комментария |
| Date format          | Нет                  | Формат даты, используемый в файле |
| Null value           | Нет                  | Текстовое значение, которое интерпретируется как Null |
...
Приемник данных, при помощи которого можно организовать потоковую обработку данных. В зависимости от настроек элемент может записывать результат трансформации в файл, хранимый в файловой системе HDFS или в таблице базы данных внешней системы.
...
| Local File Format    | Да                   | Формат файла-приемника данных |
...
| Trigger units        | Нет                  | Единица измерения заданного значения атрибута Trigger |
| Timeout              | Нет                  | Время работы элемента Stream target |
| Options              | Нет                  | Опции элемента: <br>**key** – название опции;<br>**value** – значение опции |
| Partitions           | Да                   | Список партиций (каталогов).<br>При разбиении на партиции, данные сортируются по каталогам. Количество партиций должно быть меньше, чем количество атрибутов. Например, таблицу со столбцами ID и NAME можно разбить по какому-нибудь одному атрибуту, либо ID, либо NAME |
...
Действия с объектами **"Workflow"** выполняются в разделе интерфейса «ETL/Поток обработки».
...
Для контроля версий файлов рекомендуется использовать программу [TortoiseSVN](https://tortoisesvn.net), параметры подключения к которойзадаются в настройках объектов [**Project**](#tab_project_param).

<a id='grouping_program_objects'></a>

### Группировка объектов программы
...
| Svn Enabled          | Нет                  | Включение параметра означает, что для учета версий файлов, описывающих экспортируемые объекты и внешние связи между ними, используется программа TortoiseSVN |
| Svn URL              | Обязательно при включении Svn Enabled | Url-адрес TortoiseSVN |
| Svn User Name        | Обязательно при включении Svn Enabled | Имя пользователя, используемое для входа в TortoiseSVN |
| Svn Password         | Обязательно при включении Svn Enabled | Пароль пользователя для входа в TortoiseSVN.<br>*Рекомендуется использовать скрытый способ хранения паролей (см. раздел «[Хранение паролей в системе](#storing_and_encrypting_passwords)»)* |
| Svn Commit Message   | Нет                  | Текст сообщения, которое будет записываться при выполнении операции SVN Commit |
...
| Название операции          | Описание                                                     |
| -------------------------- | :----------------------------------------------------------- |
| Загрузить проект           | Импортирует объекты, привязанные к объекту Project, из формы просмотра/редактирования которого запущена операция.<br><br>*Объект не будет загружен, если тип и название импортируемого объекта совпадает с типом и названием имеющегося объекта в программной среде* |
| Выгрузить проект           | Подсистема переноса метаданных создает комплект файлов, описывающих экспортируемые объекты, привязанные к объекту Project, из формы просмотра/редактирования которого запущена операция. Данные файлы сохраняются в каталоге:<br>`${deploy.dir}/deployments/${cust.code}/cim/MetaServer/data/projects/${project.name}`<br>и могут быть использованы для импорта в другую программную среду |
| Загрузить репозиторий      | Импортирует все объекты, созданные в программе.<br><br>*Объект не будет загружен, если тип и название импортируемого объекта совпадает с типом и названием имеющегося объекта в программной среде* |
| Выгрузить репозиторий      | Подсистема переноса метаданных создает комплект файлов, описывающих все объекты программы, независимо от привязки к объекту Project, из формы просмотра/редактирования которого запущена операция. Данные файлы сохраняются в каталоге:<br>`${deploy.dir}/deployments/${cust.code}/cim/MetaServer/data/all`<br>и могут быть использованы для импорта в другую программную среду |
| Очистить проект            | Операция удаляет объекты программы, типы и имена которых совпадают с загружаемыми.<br><br>*Если в программе существуют объекты, которые привязаны к нескольким проектам, то такие объекты не будут удалены* |
| Создать представление      | Для всех объектов Transformation и Workflow создается представление в формате json |
...
| SVN Checkout               | Операция выполняется после создания нового объекта Project для синхронизации репозитория SVN и локального каталога для хранения проектов |
| SVN Update                 | Операция обновляет копии рабочих файлов пользователя до актуальной версии |
| SVN Checkout or Update     | Выполняет операцию Checkout или Update в зависимости от наличия файлов |
| SVN Commit                 | Операция фиксирует внесенные изменения с созданием новой ревизии |
| SVN Cleanup                | Очистка lock-файлов                                          |
...
*`${deploy.dir}/deployments/${cust.code}/cim/MetaServer/data/projects/${project.name}`*
 ...
 *`${deploy.dir}/deployments/${cust.code}/cim/MetaServer/data/projects`*,
...
В системе существуют два типа Workspace:

- **Analytic workspace** - рабочая область, предназначенная для персонала, который занимается бизнес-анализом данных и в силу правил безопасности не должен иметь доступа к определенным датасетам;
...
В **Jdbc workspace** существует возможнось автоматически создать датасеты для всей базы данных по кнопке ![](Pic\OperationsButton.png)(Import schema). При этом, в настройках Jdbc workspace обязательно должны быть указаны атрибуты **Jdbc connection** и **Default schema to work**. После загрузки базы данных, на экране будут показаны датасеты в виде блоков, а в левой части экрана представлена структура Workspace. 

![](Pic\Full_workspace.png)

##### *Импорт датасетов*

В analytic workspace и Jdbc workspace можно импортировать уже созданные датасеты из других workspace. Для этого кликните по кнопке ![](Pic\ImportDatasets.png)(Import datasets), на экране появится окно выбора датасетов для импорта.<br>
...
Создание датасетов и child workspace доступно как в analytic workspace, так и в Jdbc workspace. Чтобы создать датасет и/или child workspace, нажмите кнопку ![](Pic\SelectTransformationDeploymentButton.png), расположенную рядом с соответствующим пунктом на панели со структурой workspace.<br>

<a id='sseChildWorkspaces'></a>

#### Child workspaces

**Child workspaces** - дочерние рабочие среды, данные из которых будут доступны в рабочей workspace. Независимо от типа родительской workspace, Child workspaces могут быть как analytic, так и Jdbc. 
...
- Import metadata - данная операция импортирует метаданные из источника данных. Операция доступна по кнопке ![](C:/D/SVN/DOC/MD%20manual/UserManualRU/Pic/OperationsButton.png).<br>
...
Операции доступны по кнопке ![](C:/D/SVN/DOC/MD%20manual/UserManualRU/Pic/OperationsButton.png).<br>
...
На вкладке "View" данные могут быть представленны в виде:
...
Для импорта/обновления метаданных в датавете используйте операцию "Import metadata", которая доступна по кнопке ![](Pic\OperationsButton.png) (данная операция доступна не для всех датасетов).

##### *Редактирование атрибутов датасета*

Редактирование атрибутов датасета выполняется на вкладке "Properties editor". Для перехода на вкладку выберите пункт "Properties editor" в списке, открываемом кнопкой ![](Pic/PlusButton.png) в открытом датасете, или кнопкой ![](Pic\GearButton.png)  в открытом workspace. Атрибуты датасетов описаны в соответствующих пунктах раздела ["Типы датасетов"](#sseDatasetTypes).
...
Данная система может быть использована в различных сферах бизнеса, где требуется бесприрывный анализ входящего потока событий, например: анализ поведения пользователя на страницах сайта или контроль перемещения автомобилей при организации перевозок и т.д.
...
Объекты Events processor хранят описания структур входящего и исходящего потока данных, а также  правила их обработки. 
...
| Internal state structure |            Нет            | Структура данных с промежуточными результатами обработки входящего потока, рассчитанными между событиями. Internal state structure сохраняются |
| Output structure         |            Нет            | Описание структуры исходящего потока данных                  |
| Local variables          |            Нет            | Структура данных с промежуточными результатами обработки входящего потока, расчитанными между правилами. Local variables не сохраняются |
| Rules                    |            Нет            | Список правил анализа событий<br><br>*Между собой все правила объединяются логическим оператором AND* |



##### *Настройка правил анализа событий*

Анализ входящего потока событий выполняется при помощи правил обработки, настраиваемых пользователем. Для создания и/или редактирования правил, перейдите на вкладку "Rule editor", которая открывается по кнопке ![](Pic\PlusButton.png)  . ![Events rules](Pic\Events rules.png)

Новые правила создаются по кнопке ![](Pic\SelectTransformationDeploymentButton.png), расположенной правее строки поиска (см. рисунок выше).

> *Примечание.*
>
> *Количество создаваемых правил не ограничено. Между собой все правила объединяются логическим оператором AND*



![Events new rules](Pic\Events new rules.png)





  Язык Mvel https://github.com/mvel/mvel
...
| Атрибут           | Обязательно<br>заполнение | Описание                                                     |
| :---------------- | :-----------------------: | ------------------------------------------------------------ |
| Library           |            Нет            | Название библиотеки к которой привязан объект Function. Билиотеки создаются в разделе интерфейса: "Streaming/Functions library" |
...
## Приложение 3. Неисправности и способы их устранения